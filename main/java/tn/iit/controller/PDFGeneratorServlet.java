...
doPost(request,response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String teacherName = e.getName();
		    ...
		    try (PDDocument document = new PDDocument()) {
		        ...
		        try (InputStream secondImageInputStream = secondImageUrl.openStream()) {
		            PDImageXObject secondImage = PDImageXObject.createFromByteArray(document, IOUtils.toByteArray(secondImageInputStream), "second_image");
		            int desiredWidth = 110;
		            int desiredHeight = (int) ((double) desiredWidth / secondImage.getWidth() * secondImage.getHeight());
		            contentStream.drawImage(secondImage, page.getMediaBox().getWidth() - 50 - desiredWidth, 700, desiredWidth, desiredHeight);
		        }
		        ...
		        // Draw table header
		        drawTableCell(contentStream, margin, yPosition, tableWidth, cellHeight, "Enseignant", "Date", "Heures autorisées");
		...
		private void drawTableCell(PDPageContentStream contentStream, float x, float y, float width, float height, String... cellValues) throws IOException {
		    float margin = 5;
